...
admin.initializeApp();
const PROJECT_DIR = (0, path_1.join)(__dirname, "..", "..");
function getMarkdownFiles() {
    if (!(0, fs_1.existsSync)(PROJECT_DIR))
        return [];
    return (0, fs_1.readdirSync)(PROJECT_DIR)
        .filter(f => (0, path_1.extname)(f) === ".md")
        .sort();
}
function readMarkdownFile(filename) {
    const filepath = (0, path_1.join)(PROJECT_DIR, filename);
    if (!(0, fs_1.existsSync)(filepath))
        return "";
    return (0, fs_1.readFileSync)(filepath, "utf-8");
}
function renderMarkdown(content) {
    return marked_1.marked.parse(content, { async: false });
}
function generateFileList() {
    const files = getMarkdownFiles();
    return files.map(f => {
        const name = (0, path_1.basename)(f, ".md");
        return `<li><a href="/${encodeURIComponent(f)}">${name}</a></li>`;
    }).join("");
}
function generateHtml(title, content, isFile = false) {
...
exports.authGuard = functions.https.onRequest(async (req, res) => {
    try {
        ...
        if (filename.endsWith(".md") && getMarkdownFiles().includes(filename)) {
            ...
            const html = renderMarkdown(content);
            res.send(generateHtml((0, path_1.basename)(filename, ".md"), html, true));
            return;
        }
